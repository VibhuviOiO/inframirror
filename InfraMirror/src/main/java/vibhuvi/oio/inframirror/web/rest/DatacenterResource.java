...
public class DatacenterResource {
    ...
    public ResponseEntity<DatacenterDTO> updateDatacenter(
        ...
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, datacenterDTO.getId().toString()))
            .body(datacenterDTO);
    }
    ...
    public ResponseEntity<DatacenterDTO> partialUpdateDatacenter(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, datacenterDTO.getId().toString())
        );
    }
    ...
    public ResponseEntity<List<DatacenterDTO>> getAllDatacenters(
        DatacenterCriteria criteria,
        @org.springdoc.core.annotations.ParameterObject Pageable pageable
    ) {
        LOG.debug("REST request to get Datacenters by criteria: {}", criteria);

        Page<DatacenterDTO> page = datacenterQueryService.findByCriteria(criteria, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }

    /**
     * {@code GET  /datacenters/count} : count all the datacenters.
     *
     * @param criteria the criteria which the requested entities should match.
    ...
    /**
     * {@code SEARCH  /datacenters/_search?query=:query} : search for the datacenter corresponding
     * to the query.
     *
     * @param query the query of the datacenter search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search")
    public ResponseEntity<List<DatacenterDTO>> searchDatacenters(
