...
import { getEntities, reset, searchEntities } from './tag.reducer';
  ...
  return (
    <div>
      <h2 id="tag-heading" data-cy="TagHeading">
        <Translate contentKey="infraMirrorApp.tag.home.title">Tags</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="infraMirrorApp.tag.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/tag/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="infraMirrorApp.tag.home.createLabel">Create new Tag</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('infraMirrorApp.tag.home.search')}
                />
          ...
          {tagList && tagList.length > 0 ? (
              ...
              <thead>
                <tr>
                  <th className="hand" onClick={sort('id')}>
                    <Translate contentKey="infraMirrorApp.tag.id">ID</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                  </th>
                  <th className="hand" onClick={sort('key')}>
                    <Translate contentKey="infraMirrorApp.tag.key">Key</Translate> <FontAwesomeIcon icon={getSortIconByFieldName('key')} />
                  </th>
              ...
              <tbody>
                {tagList.map((tag, i) => (
                  <tr key={`entity-${i}`} data-cy="entityTable">
                    ...
                    <td>{tag.key}</td>
                    <td>{tag.value}</td>
                    <td>{tag.entityType}</td>
                    <td>{tag.entityId}</td>
                    <td>{tag.createdBy}</td>
                    <td>{tag.createdDate ? <TextFormat type="date" value={tag.createdDate} format={APP_DATE_FORMAT} /> : null}</td>
            ...
            !loading && (
              <div className="alert alert-warning">
                <Translate contentKey="infraMirrorApp.tag.home.notFound">No Tags found</Translate>
              </div>
            )
          )}
        </InfiniteScroll>
