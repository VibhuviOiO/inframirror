 ...
 */
public class AgentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all agents").get("/api/agents").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new agent")
                        .post("/api/agents")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created agent").get("${new_agent_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created agent").delete("${new_agent_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Agent entity").exec(scn);
