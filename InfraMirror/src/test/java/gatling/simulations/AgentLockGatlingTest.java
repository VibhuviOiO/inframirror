...
public class AgentLockGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all agentLocks").get("/api/agent-locks").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new agentLock")
                        .post("/api/agent-locks")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_agentLock_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created agentLock").get("${new_agentLock_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created agentLock").delete("${new_agentLock_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AgentLock entity").exec(scn);
