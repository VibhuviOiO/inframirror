...
public class AgentMonitorGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all agentMonitors").get("/api/agent-monitors").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new agentMonitor")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_agentMonitor_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created agentMonitor").get("${new_agentMonitor_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created agentMonitor").delete("${new_agentMonitor_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AgentMonitor entity").exec(scn);
