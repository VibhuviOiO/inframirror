 ...
 */
public class ApiKeyGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all apiKeys").get("/api/api-keys").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new apiKey")
                        .post("/api/api-keys")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"keyHash\": \"SAMPLE_TEXT\"" +
                                ", \"active\": null" +
                                ", \"lastUsedDate\": \"2020-01-01T00:00:00.000Z\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_apiKey_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created apiKey").get("${new_apiKey_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created apiKey").delete("${new_apiKey_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ApiKey entity").exec(scn);
