...
public class BrandingGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all brandings").get("/api/brandings").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new branding")
                        .post("/api/brandings")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"author\": \"SAMPLE_TEXT\"" +
                                ", \"faviconPath\": \"SAMPLE_TEXT\"" +
                                ", \"logoPath\": \"SAMPLE_TEXT\"" +
                                ", \"logoWidth\": 0" +
                                ", \"logoHeight\": 0" +
                                ", \"footerTitle\": \"SAMPLE_TEXT\"" +
                                ", \"isActive\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_branding_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created branding").get("${new_branding_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created branding").delete("${new_branding_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Branding entity").exec(scn);
