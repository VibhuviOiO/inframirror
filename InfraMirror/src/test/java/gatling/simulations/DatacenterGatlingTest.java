...
public class DatacenterGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all datacenters").get("/api/datacenters").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new datacenter")
                        .post("/api/datacenters")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"code\": \"SAMPLE_TEXT\"" + ", \"name\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_datacenter_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created datacenter").get("${new_datacenter_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created datacenter").delete("${new_datacenter_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Datacenter entity").exec(scn);
