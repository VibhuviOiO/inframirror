...
public class HttpMonitorGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all httpMonitors").get("/api/http-monitors").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new httpMonitor")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_httpMonitor_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created httpMonitor").get("${new_httpMonitor_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created httpMonitor").delete("${new_httpMonitor_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the HttpMonitor entity").exec(scn);
