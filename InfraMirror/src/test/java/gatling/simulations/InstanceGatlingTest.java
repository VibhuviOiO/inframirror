 ...
 */
public class InstanceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all instances").get("/api/instances").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new instance")
                        .post("/api/instances")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"instanceType\": \"SAMPLE_TEXT\"" +
                                ", \"monitoringType\": \"SAMPLE_TEXT\"" +
                                ", \"operatingSystem\": \"SAMPLE_TEXT\"" +
                                ", \"platform\": \"SAMPLE_TEXT\"" +
                                ", \"privateIpAddress\": \"SAMPLE_TEXT\"" +
                                ", \"publicIpAddress\": \"SAMPLE_TEXT\"" +
                                ", \"tags\": null" +
                                ", \"pingEnabled\": null" +
                                ", \"pingInterval\": 0" +
                                ", \"pingTimeoutMs\": 0" +
                                ", \"pingRetryCount\": 0" +
                                ", \"hardwareMonitoringEnabled\": null" +
                                ", \"hardwareMonitoringInterval\": 0" +
                                ", \"cpuWarningThreshold\": 0" +
                                ", \"cpuDangerThreshold\": 0" +
                                ", \"memoryWarningThreshold\": 0" +
                                ", \"memoryDangerThreshold\": 0" +
                                ", \"diskWarningThreshold\": 0" +
                                ", \"diskDangerThreshold\": 0" +
                                ", \"createdAt\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"updatedAt\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"lastPingAt\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"lastHardwareCheckAt\": \"2020-01-01T00:00:00.000Z\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_instance_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created instance").get("${new_instance_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created instance").delete("${new_instance_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Instance entity").exec(scn);
