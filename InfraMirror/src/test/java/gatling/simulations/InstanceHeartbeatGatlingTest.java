...
public class InstanceHeartbeatGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all instanceHeartbeats").get("/api/instance-heartbeats").headers(headersHttpAuthenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new instanceHeartbeat")
                        .post("/api/instance-heartbeats")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"executedAt\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"heartbeatType\": \"SAMPLE_TEXT\"" +
                                ", \"success\": null" +
                                ", \"responseTimeMs\": 0" +
                                ", \"packetLoss\": 0" +
                                ", \"jitterMs\": 0" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_instanceHeartbeat_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created instanceHeartbeat").get("${new_instanceHeartbeat_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created instanceHeartbeat").delete("${new_instanceHeartbeat_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the InstanceHeartbeat entity").exec(scn);
