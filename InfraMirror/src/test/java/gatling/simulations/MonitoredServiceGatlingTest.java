...
public class MonitoredServiceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all monitoredServices").get("/api/monitored-services").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new monitoredService")
                        .post("/api/monitored-services")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"environment\": \"SAMPLE_TEXT\"" +
                                ", \"monitoringEnabled\": null" +
                                ", \"clusterMonitoringEnabled\": null" +
                                ", \"intervalSeconds\": 0" +
                                ", \"timeoutMs\": 0" +
                                ", \"retryCount\": 0" +
                                ", \"latencyWarningMs\": 0" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_monitoredService_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created monitoredService").get("${new_monitoredService_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created monitoredService").delete("${new_monitoredService_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MonitoredService entity").exec(scn);
