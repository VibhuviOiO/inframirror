...
public class PingHeartbeatGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all pingHeartbeats").get("/api/ping-heartbeats").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new pingHeartbeat")
                        .post("/api/ping-heartbeats")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_pingHeartbeat_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created pingHeartbeat").get("${new_pingHeartbeat_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created pingHeartbeat").delete("${new_pingHeartbeat_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PingHeartbeat entity").exec(scn);
