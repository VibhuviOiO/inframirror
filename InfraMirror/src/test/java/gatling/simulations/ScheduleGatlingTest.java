 ...
 */
public class ScheduleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all schedules").get("/api/schedules").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new schedule")
                        .post("/api/schedules")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created schedule").get("${new_schedule_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created schedule").delete("${new_schedule_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Schedule entity").exec(scn);
