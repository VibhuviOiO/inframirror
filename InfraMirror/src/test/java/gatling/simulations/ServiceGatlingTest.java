...
public class ServiceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all services").get("/api/services").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created service").get("${new_service_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created service").delete("${new_service_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
