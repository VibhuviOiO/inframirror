...
public class ServiceInstanceGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all serviceInstances").get("/api/service-instances").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new serviceInstance")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_serviceInstance_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created serviceInstance").get("${new_serviceInstance_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created serviceInstance").delete("${new_serviceInstance_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ServiceInstance entity").exec(scn);
