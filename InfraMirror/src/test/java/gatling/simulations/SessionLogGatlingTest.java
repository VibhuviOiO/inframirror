...
public class SessionLogGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all sessionLogs").get("/api/session-logs").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new sessionLog")
                        .post("/api/session-logs")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"sessionType\": \"SAMPLE_TEXT\"" +
                                ", \"startTime\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"endTime\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"duration\": 0" +
                                ", \"sourceIpAddress\": \"SAMPLE_TEXT\"" +
                                ", \"status\": \"SAMPLE_TEXT\"" +
                                ", \"terminationReason\": \"SAMPLE_TEXT\"" +
                                ", \"commandsExecuted\": 0" +
                                ", \"bytesTransferred\": 0" +
                                ", \"sessionId\": \"SAMPLE_TEXT\"" +
                                ", \"metadata\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_sessionLog_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created sessionLog").get("${new_sessionLog_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created sessionLog").delete("${new_sessionLog_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SessionLog entity").exec(scn);
