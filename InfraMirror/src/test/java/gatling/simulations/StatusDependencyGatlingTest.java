...
public class StatusDependencyGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all statusDependencies").get("/api/status-dependencies").headers(headersHttpAuthenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new statusDependency")
                        .post("/api/status-dependencies")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"parentType\": \"SAMPLE_TEXT\"" +
                                ", \"parentId\": 0" +
                                ", \"childType\": \"SAMPLE_TEXT\"" +
                                ", \"childId\": 0" +
                                ", \"metadata\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_statusDependency_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created statusDependency").get("${new_statusDependency_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created statusDependency").delete("${new_statusDependency_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the StatusDependency entity").exec(scn);
