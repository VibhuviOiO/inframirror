...
public class StatusPageGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all statusPages").get("/api/status-pages").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new statusPage")
                        ...
                        .body(
                            StringBody(
                                ...
                                ", \"isPublic\": null" +
                                ", \"customDomain\": \"SAMPLE_TEXT\"" +
                                ", \"logoUrl\": \"SAMPLE_TEXT\"" +
                                ", \"themeColor\": \"SAMPLE_TEXT\"" +
                                ", \"headerText\": \"SAMPLE_TEXT\"" +
                                ", \"footerText\": \"SAMPLE_TEXT\"" +
                                ", \"showResponseTimes\": null" +
                                ", \"showUptimePercentage\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_statusPage_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created statusPage").get("${new_statusPage_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created statusPage").delete("${new_statusPage_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the StatusPage entity").exec(scn);
