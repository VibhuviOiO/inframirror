...
public class StatusPageItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all statusPageItems").get("/api/status-page-items").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new statusPageItem")
                        .post("/api/status-page-items")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_statusPageItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created statusPageItem").get("${new_statusPageItem_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created statusPageItem").delete("${new_statusPageItem_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the StatusPageItem entity").exec(scn);
