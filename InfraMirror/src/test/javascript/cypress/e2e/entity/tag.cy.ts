...
describe('Tag e2e test', () => {
  ...
  const tagSample = { key: 'circa', value: 'ah kindheartedly meager', entityType: 'barring thoroughly skyline', entityId: 3993 };
  ...
  describe('new Tag page', () => {
    ...
    it('should create an instance of Tag', () => {
      cy.get(`[data-cy="key"]`).type('bah');
      cy.get(`[data-cy="key"]`).should('have.value', 'bah');

      cy.get(`[data-cy="value"]`).type('squiggly although deep');
      cy.get(`[data-cy="value"]`).should('have.value', 'squiggly although deep');

      cy.get(`[data-cy="entityType"]`).type('early speedily softly');
      cy.get(`[data-cy="entityType"]`).should('have.value', 'early speedily softly');

      cy.get(`[data-cy="entityId"]`).type('28882');
      cy.get(`[data-cy="entityId"]`).should('have.value', '28882');

      cy.get(`[data-cy="createdBy"]`).type('failing quirkily swift');
      cy.get(`[data-cy="createdBy"]`).should('have.value', 'failing quirkily swift');

      cy.get(`[data-cy="createdDate"]`).type('2025-12-03T01:33');
      cy.get(`[data-cy="createdDate"]`).blur();
      cy.get(`[data-cy="createdDate"]`).should('have.value', '2025-12-03T01:33');
