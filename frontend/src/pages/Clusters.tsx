...
function ClustersPage() {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'catalog' | 'datacenter' | 'environment' | null>(null);
  ...
  const sortedRows = useMemo(() => {
    ...
    return [...filteredRows].sort((a, b) => {
      ...
      } else if (sortBy === 'catalog') {
        aVal = catalogs?.find(c => c.id === a.catalogId)?.name?.toLowerCase() || '';
        bVal = catalogs?.find(c => c.id === b.catalogId)?.name?.toLowerCase() || '';
  ...
  }, [filteredRows, sortBy, sortDir, catalogs, datacenters, environments]);

  const handleSort = (col: 'name' | 'catalog' | 'datacenter' | 'environment') => {
    if (sortBy === col) {
  ...
  const handleAdd = async () => {
    ...
    try {
      await create.mutateAsync({ name: addName, catalogId: addCatalog, datacenterId: addDatacenter, environmentId: addEnvironment });
      setAddName(''); setAddCatalog(''); setAddDatacenter(''); setAddEnvironment(''); setAddModalOpen(false);
  ...
  const handleEdit = async (id: number) => {
    ...
    try {
      await update.mutateAsync({ id, data: { name: editName, catalogId: editCatalog, datacenterId: editDatacenter, environmentId: editEnvironment } });
      setEditingId(null); setEditName(''); setEditCatalog(''); setEditDatacenter(''); setEditEnvironment('');
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl p-8 w-full max-w-md relative">
            ...
            <div className="mb-4">
              ...
              <select
                value={addCatalog}
                onChange={e => setAddCatalog(Number(e.target.value))}
                className="w-full px-3 py-2 border-2 border-blue-200 rounded-lg bg-white dark:bg-gray-900 focus:ring-2 focus:ring-blue-300 shadow-sm"
              >
            ...
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              <thead>
                <tr className="text-left bg-blue-100 dark:bg-blue-950 animate-fade-in">
                  ...
                  <th className="px-6 py-4 text-base font-bold text-blue-800 dark:text-blue-100 cursor-pointer select-none" onClick={() => handleSort('catalog')}>Catalog</th>
                ...
                {sortedRows.map((r, idx) => (
                  <tr key={r.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                    ...
                    <td className="px-6 py-4">{editingId === r.id ? (
                      <select value={editCatalog} onChange={e => setEditCatalog(Number(e.target.value))} className="w-full px-2 py-1 border rounded">
                        <option value="">Select Catalog</option>
                        {catalogs?.map(c => (
                          <option key={c.id} value={c.id}>{c.name}</option>
                        ))}
                      ...
                      catalogs?.find(c => c.id === r.catalogId)?.name || ''
                    )}</td>
                    <td className="px-6 py-4">{editingId === r.id ? (
